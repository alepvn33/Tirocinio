...
class Verifier:
    ...
    def Verification(self,Htr_unsys,s,aux,c,rsp_I,seed_not_I):
        ...
        if b == 1:
            ...
            b_prime = self.V2(Htr_unsys,s,aux[self.I],c[self.I],rsp_I)
            if b_prime == 1:
...
def MPC_id(e,Htr_unsys,s):
    ...
    #I. COMMITMENT (Prover)
    aux, c = Prover.Commitment(prover,Htr_unsys,e)

    #II. CHALLENGE (Verifier)
    I, ch = Verifier.Challenge(verifier)

    #III. RESPONSE (Prover)
    rsp_I, seed_not_I = Prover.Response(prover,ch,I)

    #IV. VERIFICATION (Verifier)
    return Verifier.Verification(verifier,Htr_unsys,s,aux,c,rsp_I,seed_not_I)