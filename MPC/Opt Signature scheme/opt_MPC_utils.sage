...
def key_gen():
    ...
    set_random_seed()
    pk_seed = initial_seed() #Save the seed that generates the public key H
    Htr_unsys = random_matrix(Fq,n-r,r) #public matrix (only non-systematic portion)
    set_random_seed() #Set a random seed that generates the private key e so it can't be reconstructed from pk_seed
    e = FWV()

    #compute syndrome
    s = e[0:r] + e[r:n]*Htr_unsys

    return e,Htr_unsys,s,pk_seed
...
class Signer:
    ...
    def __init__(self,hash_leaf=False,s_num=0):
    ...
    def Commitment(self,Htr_unsys,e):
        ...
        h = hash(''.join(self.root_aux)) # Total hash of aux trees roots
        if self.hash_leaf:
        ...
        mt = self.T_c
        root_c = mt.get_merkle_root()
    
        return h, root_c

    #II. CHALLENGE
    def Challenge(self,mex,h,root_c):

        S = []      # list of s_num selected rounds (from 0 to N-1)
        z = []      # list of s_num selected aux (from 0 to q-1)
        
        set = Set(Integers(N))
        set_random_seed(Integer(hash(str(mex)+str(h)+str(root_c)),16))

        for i in range(0,self.s_num):
            S.append(int(set.random_element()))
            set = set.difference([S[i]])
            z.append(Fq.random_element())

        return S, z

    #III RESPONSE
    def Response(self,z,S):

        rsp_S = []              # list of s responses
        path_aux_S = []         # list of s aux for the selected rounds
        path_c_S = []           # list of s paths of the commitments (c) for the selected rounds
        
        for i in S:
            idx = S.index(i)    #Index of i within the list S
            rsp_S.append(self.P2(z[idx],int(self.seed[i],16),i))

            mt = self.T_aux[i]  
            path_aux_S.append(mt.get_proof(int(z[idx])))
            mt = self.T_c
            path_c_S.append(mt.get_proof(int(i)))
     
        path_seed = get_seed_path(S,self.starting_seed)

        return rsp_S, path_aux_S, path_c_S, path_seed

    #Main execution of the Signer
    def main(self,Htr_unsys,e,mex):

        #I. COMMITMENT
        h, root_c = self.Commitment(Htr_unsys,e)

        #II. CHALLENGE
        S, z = self.Challenge(mex,h,root_c)

        #III. RESPONSE
        rsp_S, path_aux_S, path_c_S, path_seed = self.Response(z,S)

        #Signature
        sigma = [h,root_c,rsp_S,path_aux_S,path_c_S,path_seed]
...
class Verifier:
    ...
    def Challenge_verifier(self,mex,h,root_c):
        ...
        for i in range(0,self.s_num):
            ...
            self.z.append(Fq.random_element())
    ...
    def main(self,Htr_unsys,s,sigma,mex):
        ...
        h, root_c, rsp_S, path_aux_S, path_c_S, path_seed = sigma

        self.Challenge_verifier(mex,h,root_c)

        return self.Verification(Htr_unsys,s,h,root_c,rsp_S,path_aux_S,path_c_S,path_seed)

##################################################################  

def MPC_id(e,Htr_unsys,s,mex,s_num):
    
    signer = Signer(True,s_num)       #True for abilitating hashing of input leaves
    ...
    #VERIFICATION
    ok = Verifier.main(verifier,Htr_unsys,s,sigma,mex)

    stop_time = perf_counter()

    
    #Get signature byte size
    sig_size = sys.getsizeof(sigma[0]) + sys.getsizeof(sigma[1]) #First two element of the signature are digests
    #The signature byte size must not include the bytes used for the Python list or dictionary structure 
    for j in range(0,len(sigma[2])):
        print(str(sys.getsizeof(sigma[2][j])))###################################
        sig_size += sys.getsizeof(sigma[2][j])
    for i in [3,4]:
        for j in range(0,len(sigma[i])):
            print(str(sys.getsizeof(sigma[i][j])))###################################
            sig_size += sys.getsizeof(sigma[i][j])
    for j in range(0,len(sigma[5])):
        print(str(sys.getsizeof(sigma[5][j])))###################################
        sig_size += sys.getsizeof(sigma[5][j])
    
    sig_size = 0
    
    return ok, sig_size, stop_time-start_time
